...
// HexMap 객체로부터 현재 시간을 가져와 DAY, 시간대, 시각을 화면에 표시합니다.
...
// x, y 위치에 사각형을 그린 뒤, 지정된 색상의 작은 사각형을 아이콘 대용으로 표시하고,  
// label(예: GOLD, FOOD)과 value(예: 100, 10/10)를 출력합니다.
...
// isLowHP가 true이면 붉은 테두리를, 아니면 회색 테두리를 사용합니다.
...
// 버튼 아래에 표시되는 포인트 색상을 clr로 지정하고, 버튼 텍스트를 출력합니다.
...
// 화면 상단에 배경, 플레이어의 금과 식량, 초상화, 현재 시간 및 메뉴 버튼을 렌더링합니다.
// u *UI는 ui.go에서 정의된 구조체이며, g *Game은 게임 전역 상태를 담은 객체입니다.  
// 참고: ui.go에 정의된 UI 구조체와 Game 구조체가 사용됩니다.  [3]
